...
class SquareHandler(HandlerPatch):
    def _create_patch(
        ...
        p = Rectangle(xy=(-xdescent + xoffset, -ydescent + yoffset), width=s, height=s)
        return p
...
class RectHandler(HandlerPatch):
    def _create_patch(
        ...
        return Rectangle(xy=(-xdescent, -ydescent), width=width, height=height)
...
class CircleHandler(HandlerPatch):
    def _create_patch(
        ...
        if width > height:
            ...
            yoffset = (height - width) / 2.0
        return Circle(
            xy=(-xdescent + s / 2 + xoffset, -ydescent + s / 2 + yoffset), radius=s / 2
        )
...
class BoxplotHandler(HandlerPatch):
    ...
    def _create_patch(
        ...
        box = Rectangle(
            xy=(-xdescent + w_offset * width, -ydescent + height * h_offset),
            width=width * self.box_w,
            height=height * self.box_h,
        )

        linewidth = 1
        vline = Rectangle(
            xy=(-xdescent + width / 2 - linewidth / 2, -ydescent),
            width=linewidth / 2,
            height=height,
        )

        hline = Rectangle(
            xy=(-xdescent + w_offset * width, -ydescent + height * 0.5 - linewidth / 4),
            width=width * self.box_w,
            height=linewidth / 2,
        )
